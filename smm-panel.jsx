...
export default function App() {
  ...
  return (
    <div className="root">
      ...
      <div className="mh"><button onClick={()=>setSb(true)} style={{background:"none",color:t.text,fontSize:22,padding:4}}>☰</button><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:8,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#fff",fontWeight:700}}>B</div><span className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</span></div><div className="m" style={{fontSize:12,fontWeight:600,color:t.green}}>{fN(user.balance)}</div></div>
      {sb&&<div className="ov" onClick={()=>setSb(false)}/>}
      <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}>
        <div style={{padding:mini?"16px 10px 12px":"22px 18px 14px",borderBottom:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between"}}>{!mini&&<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:34,height:34,borderRadius:10,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff"}}>B</div><div className="sb-hide"><div className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:10,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Premium SMM</div></div></div>}{mini&&<div style={{width:34,height:34,borderRadius:10,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff"}}>B</div>}<button className="sb-close" onClick={()=>setSb(false)} style={{background:"none",color:t.textMuted,fontSize:20,padding:4,alignItems:"center",justifyContent:"center"}}>✕</button>{!mini&&<button className="sb-collapse" onClick={()=>setMini(true)} style={{background:t.btnSecondary,color:t.textSoft,fontSize:14,padding:"6px 8px",alignItems:"center",justifyContent:"center",borderRadius:6,border:`1px solid ${t.btnSecBorder}`}}>⟨⟨</button>}</div>
        <div className="sb-balance" style={{margin:"14px 14px 6px",padding:"16px",borderRadius:16,background:t.balGrad,border:`1px solid ${t.balBorder}`}}><div style={{fontSize:10,color:t.accent,fontWeight:600,textTransform:"uppercase",letterSpacing:2}}>Wallet Balance</div><div className="m" style={{fontSize:22,fontWeight:700,color:t.text,marginTop:5}}>{fN(user.balance)}</div><button onClick={()=>go("funds")} style={{marginTop:12,width:"100%",padding:"10px 0",borderRadius:10,background:t.btnPrimary,color:"#fff",fontSize:13,fontWeight:600}}>+ Add Funds</button></div>
        {mini&&<div style={{padding:"12px 0",display:"flex",justifyContent:"center"}}><button onClick={()=>setMini(false)} style={{background:t.accentLight,color:t.accent,fontSize:14,padding:"8px 10px",borderRadius:6,border:`1px solid ${t.accentBorder}`}}>⟩⟩</button></div>}
        <nav style={{flex:1,padding:mini?"8px 6px":"8px 10px",display:"flex",flexDirection:"column",gap:2,overflowY:"auto"}}>{NAV.map(([id,ic,lb])=><button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:12,padding:mini?"11px 0":"11px 14px",borderRadius:10,width:"100%",textAlign:"left",justifyContent:mini?"center":"flex-start",background:pg===id?t.accentLight:"transparent",color:pg===id?t.accent:t.textSoft,fontSize:14,fontWeight:500,border:"1px solid transparent",boxShadow:pg===id?t.accentShadow:"none"}}><span style={{fontSize:16,width:20,textAlign:"center"}}>{ic}</span><span className="sb-nav-label">{lb}</span></button>)}</nav>
        <div style={{padding:mini?"10px 6px":"12px 14px",borderTop:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between"}}>{!mini&&<span style={{fontSize:12,color:t.textMuted}}>{dark?"Night":"Day"} mode</span>}<ThemeToggle dark={dark} onToggle={toggleTheme} compact={mini}/></div>
...
function NewOrd({services,onPlace,bal,t,dark}){
  ...
  <input placeholder="Search services..." value={q} onChange={e=>setQ(e.target.value)} style={{width:"100%",padding:"12px 16px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,marginBottom:12,outline:"none"}}/>
...
function Fnds({onAdd,bal,txs,t,dark}){
  ...
  const simPayFailed=(reason)=>{setPayState("failed");setPayError(reason||"Transaction declined by bank");};
    ...
    <div className="fg">
      <div>
        ...
        {payState==="popup"&&<Card dark={dark} style={{border:`1px solid ${t.accentBorder}`}}>
          ...
          <div style={{fontSize:11,fontWeight:600,color:t.textMuted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Simulate outcome:</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={simPaySuccess} style={{flex:1,padding:"12px 0",borderRadius:10,background:dark?"rgba(110,231,183,0.1)":"#ecfdf5",color:t.green,fontSize:13,fontWeight:600,border:`1px solid ${dark?"rgba(110,231,183,0.2)":"#a7f3d0"}`}}>✓ Success</button>
            <button onClick={()=>simPayFailed("Card declined — insufficient funds")} style={{flex:1,padding:"12px 0",borderRadius:10,background:dark?"rgba(252,165,165,0.1)":"#fef2f2",color:t.red,fontSize:13,fontWeight:600,border:`1px solid ${dark?"rgba(252,165,165,0.2)":"#fecaca"}`}}>✕ Decline</button>
          ...
          <div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={()=>simPayFailed("Network timeout — please try again")} style={{flex:1,padding:"10px 0",borderRadius:8,background:t.btnSecondary,color:t.textMuted,fontSize:12,border:`1px solid ${t.btnSecBorder}`}}>⏱ Timeout</button>
            <button onClick={()=>simPayFailed("OTP verification failed")} style={{flex:1,padding:"10px 0",borderRadius:8,background:t.btnSecondary,color:t.textMuted,fontSize:12,border:`1px solid ${t.btnSecBorder}`}}>🔐 OTP Fail</button>
          </div>
        </Card>}
...
function Sup({t,dark}){const [su,setSu]=useState("");const [ms,setMs]=useState("");const [oi,setOi]=useState("");const [submitting,setSubmitting]=useState(false);return <div><Hdr title="Support" sub="Open a ticket or check the FAQ" t={t}/><div className="spg"><Card dark={dark}><h3 style={{fontSize:15,fontWeight:600,color:t.text,marginBottom:16}}>Open a Ticket</h3><label style={{fontSize:11,color:t.textSoft,fontWeight:600,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Order ID (optional)</label><input placeholder="e.g. ORD-28491" value={oi} onChange={e=>setOi(e.target.value)} style={{width:"100%",padding:"12px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,marginBottom:14,outline:"none"}}/><label style={{fontSize:11,color:t.textSoft,fontWeight:600,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Subject</label><input placeholder="Brief description" value={su} onChange={e=>setSu(e.target.value)} style={{width:"100%",padding:"12px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,marginBottom:14,outline:"none"}}/><label style={{fontSize:11,color:t.textSoft,fontWeight:600,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Message</label><textarea rows={4} placeholder="Describe your issue..." value={ms} onChange={e=>setMs(e.target.value)} style={{width:"100%",padding:"12px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,marginBottom:16,outline:"none",resize:"vertical"}}/><button disabled={submitting} style={{width:"100%",padding:"14px 0",borderRadius:12,background:submitting?(dark?"#333":"#ccc"):t.btnPrimary,color:"#fff",fontSize:15,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",gap:8}} onClick={()=>{if(su&&ms&&!submitting){setSubmitting(true);setTimeout(()=>{setSubmitting(false);setSu("");setMs("");setOi("");},1000)}}}>{submitting&&<span style={{width:16,height:16,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .6s linear infinite"}}/>}{submitting?"Submitting...":"Submit Ticket"}</button></Card><Card d={.1} dark={dark}><h3 style={{fontSize:15,fontWeight:600,color:t.text,marginBottom:16}}>FAQ</h3>{[["How fast are orders?","Most start in minutes. 0-24 hrs typical."],["What if followers drop?","Refill-guaranteed services auto-replenish."],["Is my account safe?","We never ask for passwords. Public URLs only."],["Can I get a refund?","Yes for undelivered/canceled orders."],["Payment methods?","Paystack — cards, bank transfer, USSD. Min ₦500."]].map(([q,a],i)=><details key={i} style={{marginBottom:6}}><summary style={{padding:"11px 0",color:t.text,fontSize:13,fontWeight:500,cursor:"pointer",borderBottom:`1px solid ${t.surfaceBorder}`,listStyle:"none"}}>{q}</summary><div style={{padding:"6px 0 14px",color:t.textSoft,fontSize:13,lineHeight:1.6}}>{a}</div></details>)}</Card></div></div>;}
