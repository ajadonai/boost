...
export async function recoverStalePendingPayments() {
  ...
  const stats = { checked: staleTxns.length, recovered: 0, expired: 0, errors: [] };
  ...
  log.info('Payment Recovery', `Checked ${stats.checked}: ${stats.recovered} recovered, ${stats.expired} expired`);
  return stats;
}
