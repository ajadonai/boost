...
const ROLE_PAGES = {
  ...
  admin: ['overview','orders','users','services','api','payments','tickets','activity','alerts','analytics','coupons','notifications','maintenance'],
...
export async function requireAdmin(requiredPage = null, requireWrite = false) {
  ...
  if (requiredPage) {
    ...
    if (pages !== '*' && !pages.includes(requiredPage)) {
