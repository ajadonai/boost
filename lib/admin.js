...
export function getAdminWritePages(admin) {
  // Owner/superadmin always have full write
  if (admin.role === 'owner' || admin.role === 'superadmin') return '*';

  // Custom pages don't grant extra write — write is role-bound
  // But if custom pages REMOVED a page, they can't write to it either
  const roleWrite = ROLE_WRITE_PAGES[admin.role] || [];
  const viewPages = getAdminPages(admin);

  if (viewPages === '*') return roleWrite;
  // Can only write to pages they can also view
  return roleWrite.filter(p => viewPages === '*' || viewPages.includes(p));
}
