...
export default function AdminPanel(){
  ...
  <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}><div style={{padding:mini?"10px 0":"16px 14px 12px",borderBottom:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between",flexDirection:mini?"column":"row",gap:mini?6:0}}>
  {!mini&&<button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:10,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><div style={{width:30,height:30,borderRadius:9,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff"}}>B</div><div className="sb-hide"><div className="serif" style={{fontSize:16,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:9,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Admin Panel</div></div></button>}
  ...
  <button className="sb-close" onClick={()=>setSb(false)} style={{background:"none",color:t.textMuted,fontSize:20,padding:4}}>✕</button>
  {!mini&&<button className="sb-collapse" onClick={()=>setMini(true)} style={{background:t.btnSecondary,color:t.textSoft,fontSize:14,padding:"6px 8px",borderRadius:6,border:`1px solid ${t.btnSecBorder}`}}>⟨⟨</button>}
  ...
  <nav style={{flex:1,paddingTop:6,paddingBottom:6,paddingLeft:mini?4:10,paddingRight:0,display:"flex",flexDirection:"column",gap:2,overflow:"visible"}}>{NAV.map(([id,ic,lb])=>{if(id.startsWith("---"))return <div key={id} style={{height:1,background:t.surfaceBorder,margin:mini?"4px 4px":"4px 14px"}}/>;const act=pg===id;return <button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:11,paddingTop:8,paddingBottom:8,paddingRight:act?0:(mini?0:14),paddingLeft:act?(mini?0:11):(mini?0:14),width:"100%",textAlign:"left",justifyContent:mini?"center":"flex-start",fontSize:12,fontWeight:act?600:400,overflow:"visible",border:"none",background:act?"linear-gradient(135deg, #c47d8e, #a3586b)":"transparent",color:act?"#fff":t.textSoft,borderLeft:act?"3px solid #8b4a5e":"3px solid transparent",borderRadius:act?(mini?"0 7px 7px 0":"0 10px 10px 0"):"0 9px 9px 0",marginRight:act?(mini?-6:-8):mini?0:10,position:act?"relative":"static",zIndex:act?3:"auto"}}><span style={{fontSize:mini?13:14,width:18,textAlign:"center"}}>{ic}</span><span className="sb-nav-label">{lb}</span></button>})}</nav>
  ...
  {!mini&&<div style={{padding:"10px 14px",borderTop:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",gap:8}}>
    <div style={{width:28,height:28,borderRadius:"50%",background:role.color+"33",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:role.color,flexShrink:0}}>{currentAdmin.name[0]}</div>
