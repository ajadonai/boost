...
export default function AdminPanel(){
  ...
  ]);const [activityLog,setActivityLog]=useState([]);const [adminList,setAdminList]=useState([]);const [sb,setSb]=useState(false);const [toast,setToast]=useState(null);const [currentAdmin,setCurrentAdmin]=useState({name:"Admin",role:"superadmin",email:"admin@boostpanel.ng",id:"temp"});const [loading,setLoading]=useState(true);
  ...
  <div style={{padding:"16px 14px",borderBottom:`0.5px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
    <button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:8,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><div style={{width:30,height:30,borderRadius:8,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff"}}>B</div><div><div style={{fontSize:15,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:9,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Admin Panel</div></div></button>
