...
export default function AdminPanel(){
  ...
  <div style={{padding:"16px 14px",borderBottom:`0.5px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
    <button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:8,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><NitroLogo size={32} variant="mark"/><div style={{display:"flex",alignItems:"baseline",gap:4}}><span style={{fontSize:17,fontWeight:700,color:t.text,letterSpacing:1}}>NITRO</span><span style={{fontSize:10,fontWeight:600,color:t.textMuted}}>Admin</span></div></button>
...
function UsersPage({t,dark,users,Btn,FilterBtn,notify,role}){
  const canAct=role==="superadmin"||role==="admin";
  const [uPage,setUPage]=useState(1);const [uPerPage,setUPerPage]=useState(10);const [f,setF]=useState("all");const [q,setQ]=useState("");const [creditId,setCreditId]=useState(null);const [creditAmt,setCreditAmt]=useState("");const list=users.filter(u=>(f==="all"||(f==="active"&&u.status==="Active")||(f==="suspended"&&u.status==="Suspended"))&&(!q||u.name.toLowerCase().includes(q.toLowerCase())||u.email.toLowerCase().includes(q.toLowerCase())));const doCredit=async(u)=>{if(Number(creditAmt)>0){try{const r=await fetch("/api/admin/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"credit",userId:u.id,amount:Number(creditAmt)})});const d=await r.json();if(r.ok){notify(d.message||`Credited to ${u.name}`);setCreditId(null);setCreditAmt("");}else notify(d.error||"Failed",true);}catch{notify("Credit failed",true);}}};return <div><Hdr title="Users" sub={`${users.length} registered`} t={t}/><div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}><FilterBtn active={f==="all"} onClick={()=>{setF("all");setUPage(1);}}>All ({users.length})</FilterBtn><FilterBtn active={f==="active"} onClick={()=>{setF("active");setUPage(1);}}>Active ({users.filter(u=>u.status==="Active").length})</FilterBtn><FilterBtn active={f==="suspended"} onClick={()=>{setF("suspended");setUPage(1);}}>Suspended ({users.filter(u=>u.status==="Suspended").length})</FilterBtn></div><input placeholder="Search users..." value={q} onChange={e=>setQ(e.target.value)} style={{width:"100%",maxWidth:400,padding:"10px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:13,marginBottom:16,outline:"none"}}/>{list.slice((uPage-1)*uPerPage,uPage*uPerPage).map(u=><Card key={u.id} dark={dark} style={{marginBottom:10,padding:16}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}><div style={{display:"flex",alignItems:"center",gap:12,flex:1,minWidth:180}}><div style={{width:38,height:38,borderRadius:"50%",background:`hsl(${u.id*45},40%,${dark?30:60}%)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:700,color:"#fff",flexShrink:0}}>{u.name[0]}</div><div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontSize:14,fontWeight:600,color:t.text}}>{u.name}</span><Badge s={u.status} dark={dark}/></div><div style={{fontSize:12,color:t.textMuted,marginTop:2}}>{u.email}</div></div></div><div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}><div style={{textAlign:"center"}}><div className="m" style={{fontSize:14,fontWeight:700,color:t.green}}>{fN(u.balance)}</div><div style={{fontSize:10,color:t.textMuted}}>Balance</div></div><div style={{textAlign:"center"}}><div className="m" style={{fontSize:14,fontWeight:700,color:t.text}}>{u.orders}</div><div style={{fontSize:10,color:t.textMuted}}>Orders</div></div>{canAct&&<div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Btn onClick={()=>{setCreditId(creditId===u.id?null:u.id);setCreditAmt("");}}>Credit</Btn><Btn onClick={()=>fetch("/api/admin/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:u.status==="Active"?"suspend":"activate",userId:u.id})}).then(r=>r.json()).then(d=>notify(d.message||"Done")).catch(()=>notify("Failed",true))} style={{color:u.status==="Active"?t.red:t.green}}>{u.status==="Active"?"🚫 Suspend":"✓ Activate"}</Btn></div>}</div></div>{creditId===u.id&&<div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${t.surfaceBorder}`,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><input type="number" placeholder="Amount" value={creditAmt} onChange={e=>setCreditAmt(e.target.value)} className="m" style={{flex:1,minWidth:120,padding:"10px 12px",borderRadius:8,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,outline:"none"}}/>{[1000,5000,10000,50000].map(p=><button key={p} onClick={()=>setCreditAmt(String(p))} className="m" style={{padding:"8px 10px",borderRadius:8,fontSize:11,fontWeight:600,background:Number(creditAmt)===p?t.accentLight:t.btnSecondary,color:Number(creditAmt)===p?t.accent:t.textSoft,border:`1px solid ${t.btnSecBorder}`}}>₦{p.toLocaleString()}</button>)}<Btn primary onClick={()=>{if(Number(creditAmt)>0){const btn=event?.target;if(btn)btn.disabled=true;doCredit(u);}}}>Credit {creditAmt?fN(Number(creditAmt)):""}</Btn><button onClick={()=>setCreditId(null)} style={{background:"none",color:t.textMuted,fontSize:16,padding:4}}>✕</button></div>}</Card>)}</div>;}

function ServiceMgmt({t,dark,services,Btn,notify}){
  const [q,setQ]=useState("");const [cat,setCat]=useState("all");const [pg,setPg]=useState(1);const pp=20;
  const cats=["all",...new Set(services.map(s=>s.category))];
  const filtered=services.filter(s=>(cat==="all"||s.category===cat)&&(!q||s.name.toLowerCase().includes(q.toLowerCase())));
  const paged=filtered.slice((pg-1)*pp,pg*pp);const totalPages=Math.ceil(filtered.length/pp);
  const [markup,setMarkup]=useState("54");
  const applyMarkup=async()=>{try{const r=await fetch("/api/admin/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({settings:{defaultMarkup:markup}})});const d=await r.json();if(r.ok)notify(`Global markup set to ${markup}%`);else notify(d.error||"Failed",true);}catch{notify("Failed",true);}};
  return <div><Hdr title="Services" sub={`${services.length} configured`} t={t} action={<div style={{display:"flex",gap:8}}><Btn primary onClick={async()=>{notify("Syncing...");try{const r=await fetch("/api/admin/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"sync"})});const d=await r.json();if(d.success)notify(`Synced: ${d.created} new, ${d.updated} updated`);else notify(d.error||"Sync failed",true);}catch{notify("Sync failed",true);}}}>Sync from MTP</Btn></div>}/><div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}><input placeholder="Search services..." value={q} onChange={e=>{setQ(e.target.value);setPg(1);}} style={{flex:1,minWidth:200,padding:"10px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:13,outline:"none"}}/><select value={cat} onChange={e=>{setCat(e.target.value);setPg(1);}} style={{padding:"10px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:13,outline:"none"}}>{cats.map(c=><option key={c} value={c}>{c==="all"?`All (${services.length})`:c}</option>)}</select></div><div style={{fontSize:12,color:t.textMuted,marginBottom:10}}>Showing {paged.length} of {filtered.length} services{cat!=="all"?` in ${cat}`:""}</div><div className="dtable"><Card style={{padding:0,overflow:"hidden",overflowX:"auto"}} dark={dark}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{borderBottom:`1px solid ${t.surfaceBorder}`}}>{["ID","Service","API","Cost/1K","Sell/1K","Markup","Refill","Status"].map(h=><th key={h} style={{padding:"12px 14px",textAlign:"left",fontSize:10,color:t.textMuted,fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>{h}</th>)}</tr></thead><tbody>{paged.map(s=><tr key={s.id} style={{borderBottom:`1px solid ${dark?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.04)"}`}}><td className="m" style={{padding:"12px 14px",color:t.textMuted}}>{s.id}</td><td style={{padding:"12px 14px",color:t.text}}>{s.name}</td><td className="m" style={{padding:"12px 14px",color:t.textMuted}}>{s.apiId}</td><td className="m" style={{padding:"12px 14px",color:t.red}}>{fN(s.costPer1k)}</td><td className="m" style={{padding:"12px 14px",color:t.green}}>{fN(s.sellPer1k)}</td><td className="m" style={{padding:"12px 14px",color:"#e0a458",fontWeight:600}}>{s.markup}%</td><td style={{padding:"12px 14px",color:s.refill?t.green:t.textMuted}}>{s.refill?"✓":"—"}</td><td style={{padding:"12px 14px"}}><button onClick={async()=>{try{const r=await fetch("/api/admin/services",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"toggle",serviceId:s.id})});const d=await r.json();if(d.success)notify(`${s.name} ${d.enabled?"enabled":"disabled"}`);else notify(d.error||"Failed",true);}catch{notify("Failed",true);}}} style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:4,background:s.enabled?t.green+"18":t.textMuted+"18",color:s.enabled?t.green:t.textMuted,border:"none",cursor:"pointer"}}>{s.enabled?"Active":"Off"}</button></td></tr>)}</tbody></table></Card></div><div className="mcard">{paged.map(s=><Card key={s.id} dark={dark} style={{marginBottom:10,padding:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:500,color:t.text,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</span><span style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:4,background:s.enabled?t.green+"18":t.textMuted+"18",color:s.enabled?t.green:t.textMuted}}>{s.enabled?"Active":"Off"}</span></div><div style={{display:"flex",gap:12,fontSize:11,color:t.textMuted,flexWrap:"wrap"}}><span>Cost: <span className="m" style={{color:t.red}}>{fN(s.costPer1k)}</span></span><span>Sell: <span className="m" style={{color:t.green}}>{fN(s.sellPer1k)}</span></span><span>Markup: <span className="m" style={{color:"#e0a458"}}>{s.markup}%</span></span></div></Card>)}</div>{totalPages>1&&<div style={{display:"flex",justifyContent:"center",gap:6,marginTop:14}}>{Array.from({length:Math.min(totalPages,10)},(_,i)=>i+1).map(p=><button key={p} onClick={()=>setPg(p)} style={{padding:"6px 10px",borderRadius:6,fontSize:12,fontWeight:pg===p?600:400,background:pg===p?t.accent+"20":"transparent",color:pg===p?t.accent:t.textMuted,border:`0.5px solid ${pg===p?t.accent+"40":t.surfaceBorder}`}}>{p}</button>)}{totalPages>10&&<span style={{fontSize:12,color:t.textMuted,padding:"6px"}}>...{totalPages}</span>}</div>}<Card dark={dark} style={{marginTop:16}}><h3 style={{fontSize:15,fontWeight:600,color:t.text,marginBottom:12}}>Global Markup</h3><div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}><input value={markup} onChange={e=>setMarkup(e.target.value.replace(/[^0-9]/g,"").slice(0,3))} className="m" style={{width:80,padding:"10px 14px",borderRadius:10,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:16,fontWeight:600,textAlign:"center",outline:"none"}}/><span style={{fontSize:14,color:t.textSoft}}>%</span><Btn primary onClick={applyMarkup}>Apply</Btn></div></Card></div>;}
    ...
    <Card dark={dark}>
      ...
      <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${t.surfaceBorder}`}}>
        <div style={{padding:"32px 24px",background:dark?"#060912":"#f0ece6",textAlign:"center"}}>
          ...
          <div style={{display:"inline-flex",alignItems:"center",gap:8,marginBottom:14}}><NitroLogo size={28} variant="mark"/><span className="serif" style={{fontSize:16,fontWeight:600,color:t.text}}>Nitro</span></div>
