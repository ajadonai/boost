...
const Badge = ({ s, dark }) => { const v = stColors(dark)[s]||stColors(dark).Canceled; return <span style={{ fontSize: 11, fontWeight: 500, padding: "3px 10px", borderRadius: 6, background: v[0], color: v[1], border: `0.5px solid ${v[2]}`, whiteSpace: "nowrap" }}>{s}</span>; };
...
export default function App() {
  ...
  return (
    <div className="root">
      ...
      <style>{`*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}.root{min-height:100vh;background:${t.bg};color:${t.text};font-family:'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;transition:background 1.5s cubic-bezier(.4,0,.2,1),color 1.2s ease}input,select,textarea{font-family:inherit}button{cursor:pointer;font-family:inherit;border:none}.m{font-family:'JetBrains Mono',monospace}@keyframes si{from{transform:translateX(80px);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes fu{from{transform:translateY(14px);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes spin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:${dark?"#2a2a2a":"#ccc"};border-radius:3px}.sg{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.g2{display:grid;grid-template-columns:1.5fr 1fr;gap:16px}.og{display:grid;grid-template-columns:1fr 320px;gap:16px}.fg{display:grid;grid-template-columns:1fr 1fr;gap:16px}.rg{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.spg{display:grid;grid-template-columns:1fr 1fr;gap:16px}.layout{display:flex;min-height:100vh}.sb{width:200px;background:${t.surface};border-right:0.5px solid ${t.surfaceBorder};display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:100;transition:background 1.5s cubic-bezier(.4,0,.2,1),border-color 1.5s ease;overflow:visible}.mn{margin-left:200px;margin-right:240px;padding:28px 32px;min-height:100vh;flex:1;position:relative;z-index:1}.ad{width:240px;position:fixed;top:0;right:0;bottom:0;background:${t.surface};border-left:0.5px solid ${t.surfaceBorder};padding:16px;z-index:50;transition:background 1.5s cubic-bezier(.4,0,.2,1),border-color 1.5s ease}.ov{display:none}.mh{display:none}.sb-close{display:none}.oth,.otr{display:grid;grid-template-columns:110px 1.5fr 1fr 90px 110px 90px 100px;padding:14px 20px;align-items:center}.ocm{display:none}.sth,.str{display:grid;grid-template-columns:40px 1.5fr 100px 80px 80px 70px 70px;padding:10px 16px;align-items:center}.scm{display:none}.pf-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px}@media(max-width:1024px){.sg{grid-template-columns:repeat(2,1fr)}.g2,.og,.fg,.spg{grid-template-columns:1fr}.rg{grid-template-columns:repeat(2,1fr)}.ad{display:none}.mn{margin-right:0;padding:24px 20px}}@media(max-width:768px){.sb{transform:translateX(-100%);width:260px}.sb.open{transform:translateX(0)}.ov{display:block;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:90}.mh{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:${t.surface};border-bottom:0.5px solid ${t.surfaceBorder};position:sticky;top:0;z-index:80}.mn{margin-left:0;margin-right:0;padding:16px}.sg{grid-template-columns:repeat(2,1fr);gap:10px}.rg{grid-template-columns:1fr}.oth,.otr{display:none}.ocm{display:block}.sth,.str{display:none}.scm{display:block}.sb-close{display:flex}}@media(max-width:400px){.sg{grid-template-columns:1fr}.mn{padding:12px}}`}</style>
      ...
      <aside className={`sb${sb?" open":""}`}>
        <div style={{padding:"16px 14px",borderBottom:`0.5px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:8,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><div style={{width:30,height:30,borderRadius:8,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff"}}>B</div><div><div style={{fontSize:15,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:9,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Premium SMM</div></div></button>
          <button className="sb-close" onClick={()=>setSb(false)} style={{background:"none",color:t.textMuted,fontSize:18,padding:4,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        ...
        <nav style={{flex:1,paddingTop:6,paddingBottom:6,paddingLeft:8,paddingRight:0,display:"flex",flexDirection:"column",gap:1,overflow:"visible"}}>{NAV.map(([id,ic,lb])=>{const act=pg===id;return <button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:10,paddingTop:8,paddingBottom:8,paddingRight:act?0:12,paddingLeft:act?9:12,width:"100%",textAlign:"left",fontSize:13,fontWeight:act?600:400,overflow:"visible",border:"none",background:act?"linear-gradient(135deg, #c47d8e, #a3586b)":"transparent",color:act?"#fff":t.textSoft,borderLeft:act?"3px solid #8b4a5e":"3px solid transparent",borderRadius:act?"0 10px 10px 0":"0 8px 8px 0",marginRight:act?-8:8,position:act?"relative":"static",zIndex:act?3:"auto"}}><span style={{fontSize:15,width:18,textAlign:"center"}}>{ic}</span>{lb}</button>})}</nav>
...
const Hdr=({title,sub,action,t})=><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,animation:"fu .4s ease",flexWrap:"wrap",gap:12}}><div><h1 style={{fontSize:22,fontWeight:600,color:t.text,letterSpacing:"-0.2px"}}>{title}</h1>{sub&&<p style={{fontSize:13,color:t.textSoft,marginTop:4}}>{sub}</p>}</div>{action}</div>;
const Stat=({l,v,c,d=0,t,dark})=><div style={{background:dark?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.02)",borderRadius:10,padding:16,animation:`fu 0.5s ease ${d}s both`}}><div style={{fontSize:11,color:t.textMuted,fontWeight:500,textTransform:"uppercase",letterSpacing:1.5}}>{l}</div><div className="m" style={{fontSize:20,fontWeight:600,color:c||t.text,marginTop:6}}>{v}</div></div>;
