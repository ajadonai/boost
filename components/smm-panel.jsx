    ...
    <div className="root">
      ...
      <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}>
        ...
        <div style={{padding:mini?"12px 0":"16px 14px",borderBottom:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between",flexDirection:mini?"column":"row",gap:mini?6:0}}>
          {!mini&&<button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:8,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><div style={{width:32,height:32,borderRadius:9,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:700,color:"#fff"}}>B</div><div className="sb-hide"><div className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:9,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Premium SMM</div></div></button>}
        ...
        {/* Balance */}
        {!mini&&<div className="sb-balance" style={{padding:"10px 14px",borderBottom:`1px solid ${t.surfaceBorder}`}}><div style={{fontSize:9,color:t.textMuted,fontWeight:600,textTransform:"uppercase",letterSpacing:2}}>Balance</div><div className="m" style={{fontSize:18,fontWeight:700,color:t.green,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fNc(user.balance)}</div></div>}
        {mini&&<div style={{padding:"6px 0",borderBottom:`1px solid ${t.surfaceBorder}`,textAlign:"center"}}><div className="m" style={{fontSize:10,fontWeight:700,color:t.green,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 4px"}}>{fNc(user.balance)}</div></div>}
        {/* Nav */}
        <nav style={{flex:1,paddingTop:6,paddingBottom:6,paddingLeft:mini?4:8,paddingRight:0,display:"flex",flexDirection:"column",gap:1,overflow:"visible"}}>{NAV.map(([id,ic,lb])=>{const act=pg===id;return <button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:10,paddingTop:8,paddingBottom:8,paddingRight:act?0:(mini?0:12),paddingLeft:act?(mini?0:9):(mini?0:12),width:"100%",textAlign:"left",justifyContent:mini?"center":"flex-start",fontSize:14,fontWeight:act?600:400,overflow:"visible",border:"none",background:act?"linear-gradient(135deg, #c47d8e, #a3586b)":"transparent",color:act?"#fff":t.textSoft,borderLeft:act?"3px solid #8b4a5e":"3px solid transparent",borderRadius:act?(mini?"0 8px 8px 0":"0 10px 10px 0"):"0 8px 8px 0",marginRight:act?(mini?-6:-8):mini?0:8,position:act?"relative":"static",zIndex:act?3:"auto"}}><span style={{fontSize:mini?15:16,width:18,textAlign:"center"}}>{ic}</span><span className="sb-nav-label">{lb}</span></button>})}</nav>
        {/* Theme + Logout */}
        ...
        {!mini&&<div style={{padding:"10px 14px",borderTop:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:28,height:28,borderRadius:"50%",background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff",flexShrink:0}}>{user.name[0]}</div>
