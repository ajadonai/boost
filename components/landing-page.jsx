...
export default function Landing(){
  ...
  const heroLoginSubmit=async()=>{
    ...
    setHeroLoading(true);
    try{const res=await fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:heroMethod==="email"?heroEmail:`+234${heroEmail}`,password:heroPw})});const data=await res.json();if(!res.ok){setHeroError(data.error||"Login failed");setHeroLoading(false);return;}if(!data.user.emailVerified){window.location.href="/verify";return;}window.location.href="/dashboard";}catch{setHeroError("Something went wrong.");setHeroLoading(false);}
  };
  const heroSignupSubmit=()=>{
    setHeroError("");if(!heroName){setHeroError("Please enter your name");return;}if(!heroEmail){setHeroError("Please enter your email");return;}
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(heroEmail)){setHeroError("Please enter a valid email");return;}
    setHeroSignupData({name:heroName,email:heroEmail});setModal("signup");
  };
  ...
  const t=useMemo(()=>({
    ...
    green:dark?"#6ee7b7":"#059669",red:dark?"#fca5a5":"#dc2626",
        ...
        <section id="hero" className="snap-section" style={{overflow:"hidden",background:t.heroBg,position:"relative",display:"flex",flexDirection:"column"}}>
          ...
          <div className="hero-split">
            ...
            <div className="hero-right">
              <div style={{background:dark?"rgba(17,22,40,0.95)":"rgba(255,255,255,0.95)",border:`1px solid ${dark?"rgba(255,255,255,.08)":"rgba(0,0,0,.06)"}`,borderRadius:20,padding:"28px 26px",backdropFilter:"blur(20px)",boxShadow:dark?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(0,0,0,0.1)",height:520}}>
                ...
                <h2 style={{fontSize:22,fontWeight:700,color:t.text,textAlign:"center",marginBottom:3}}>{heroAuth==="login"?"Welcome back":heroAuth==="forgot"?"Forgot password?":"Create Account"}</h2>
                <p style={{fontSize:13,color:t.textSoft,textAlign:"center",marginBottom:20,fontWeight:430}}>{heroAuth==="login"?"Log in to your Nitro account":heroAuth==="forgot"?"Enter your email for a reset link":"Join "+siteStats.users+" Nigerian creators"}</p>
                {heroAuth!=="forgot"&&<div style={{display:"flex",marginBottom:16,background:dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.03)",borderRadius:10,padding:3,border:`1px solid ${t.surfaceBorder}`}}><button onClick={()=>setHeroMethod("email")} style={{flex:1,padding:"8px 0",borderRadius:8,fontSize:13,fontWeight:550,background:heroMethod==="email"?t.accentLight:"transparent",color:heroMethod==="email"?t.accent:t.textMuted}}>Email</button><button onClick={()=>setHeroMethod("phone")} style={{flex:1,padding:"8px 0",borderRadius:8,fontSize:13,fontWeight:550,background:heroMethod==="phone"?t.accentLight:"transparent",color:heroMethod==="phone"?t.accent:t.textMuted}}>Phone</button></div>}
                {heroAuth==="signup"&&<><label style={{fontSize:10,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Full Name</label><input placeholder="Your name" value={heroName} onChange={e=>setHeroName(e.target.value.replace(/[^a-zA-Z\u00C0-\u017F\s'\-\.]/g,"").slice(0,100))} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,outline:"none",marginBottom:12,fontFamily:"inherit"}}/></>}
                {heroAuth!=="forgot"&&<><label style={{fontSize:10,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>{heroMethod==="email"?"Email Address":"Phone Number"}</label>
                <input placeholder={heroMethod==="email"?"you@example.com":"8012345678"} value={heroEmail} onChange={e=>setHeroEmail(heroMethod==="email"?e.target.value.trim().toLowerCase().slice(0,254):e.target.value.replace(/\D/g,"").slice(0,11))} type={heroMethod==="email"?"email":"tel"} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,outline:"none",marginBottom:12,fontFamily:"inherit"}}/></>}
                {heroAuth==="login"&&<><label style={{fontSize:10,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Password</label><input placeholder="Enter password" type="password" value={heroPw} onChange={e=>setHeroPw(e.target.value.slice(0,128))} onKeyDown={e=>{if(e.key==="Enter")heroLoginSubmit()}} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:14,outline:"none",marginBottom:14,fontFamily:"inherit"}}/><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}><input type="checkbox" style={{accentColor:t.accent,width:14,height:14}}/><span style={{fontSize:12,color:t.textSoft}}>Remember me</span></label><button onClick={()=>{setHeroAuth("forgot");setHeroError("")}} style={{background:"none",color:t.accent,fontSize:12,fontWeight:500}}>Forgot password?</button></div></>}
                ...
                <button onClick={heroAuth==="login"?heroLoginSubmit:heroAuth==="forgot"?heroForgotSubmit:heroSignupSubmit} disabled={heroLoading} style={{width:"100%",padding:"13px 0",borderRadius:12,background:heroLoading?"#999":t.btnPrimary,color:"#fff",fontSize:15,fontWeight:700,marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:heroLoading?.7:1}}>{heroLoading&&<span style={{width:16,height:16,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .6s linear infinite"}}/>}{heroAuth==="login"?(heroLoading?"Logging in...":"Log In"):heroAuth==="forgot"?(heroLoading?"Sending...":"Send Reset Link"):(heroLoading?"Please wait...":"Continue →")}</button>
                <div style={{textAlign:"center",fontSize:13,color:t.textSoft}}>{heroAuth==="login"?"Don't have an account? ":heroAuth==="forgot"?"Remember your password? ":"Already have an account? "}<button onClick={()=>{setHeroAuth(heroAuth==="forgot"?"login":heroAuth==="login"?"signup":"login");setHeroError("")}} style={{background:"none",color:t.accent,fontWeight:600,fontSize:13}}>{heroAuth==="forgot"?"Log In":heroAuth==="login"?"Sign Up Free":"Log In"}</button></div>
