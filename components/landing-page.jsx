    ...
    <div className="root" suppressHydrationWarning style={{height:"100dvh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <nav className="main-nav" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 36px",height:60,background:dark?"rgba(12,16,34,.95)":"rgba(255,255,255,.95)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${t.surfaceBorder}`,flexShrink:0,zIndex:100}}>
        ...
        <div className="nav-desktop" style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <button onClick={toggleTheme} className="nav-theme" style={{width:44,height:24,borderRadius:12,background:dark?"#c47d8e":"rgba(0,0,0,0.08)",position:"relative",transition:"all .3s",flexShrink:0}}><div style={{width:18,height:18,borderRadius:"50%",background:"#fff",position:"absolute",top:3,left:dark?23:3,transition:"left .3s cubic-bezier(.2,.8,.2,1)",boxShadow:"0 1px 4px rgba(0,0,0,.2)"}}/></button>
          ...
          <button onClick={()=>setModal("signup")} className="nav-cta" style={{padding:"9px 20px",borderRadius:10,background:t.btnPrimary,color:"#fff",fontSize:13,fontWeight:600}}>Get Started</button>
        </div>
        {/* Mobile hamburger */}
        <div className="nav-hamburger" style={{display:"none"}}>
          <button onClick={toggleTheme} style={{width:40,height:24,borderRadius:12,background:dark?"#c47d8e":"rgba(0,0,0,0.08)",position:"relative",transition:"all .3s",flexShrink:0}}><div style={{width:16,height:16,borderRadius:"50%",background:"#fff",position:"absolute",top:4,left:dark?21:3,transition:"left .3s cubic-bezier(.2,.8,.2,1)",boxShadow:"0 1px 4px rgba(0,0,0,.2)"}}/></button>
          <button onClick={()=>setMobNav(!mobNav)} style={{background:"none",padding:8,display:"flex",flexDirection:"column",gap:4,justifyContent:"center"}}>
            <div style={{width:20,height:1.5,background:t.text,borderRadius:1,transition:"all .3s",transform:mobNav?"rotate(45deg) translateY(2.75px)":"none"}}/>
            <div style={{width:mobNav?0:14,height:1.5,background:t.text,borderRadius:1,transition:"all .3s",opacity:mobNav?0:1}}/>
            <div style={{width:20,height:1.5,background:t.text,borderRadius:1,transition:"all .3s",transform:mobNav?"rotate(-45deg) translateY(-2.75px)":"none"}}/>
          </button>
        </div>
      </nav>
      ...
      {/* Mobile menu drawer */}
      <div className="mob-menu" style={{display:"none",flexDirection:"column",gap:0,overflow:"hidden",maxHeight:mobNav?400:0,transition:"max-height .35s cubic-bezier(.16,1,.3,1)",background:dark?"rgba(12,16,34,.98)":"rgba(255,255,255,.98)",backdropFilter:"blur(16px)",borderBottom:mobNav?`1px solid ${t.surfaceBorder}`:"none",zIndex:99}}>
        <div style={{padding:"16px 24px",display:"flex",flexDirection:"column",gap:4}}>
          {["Services","Pricing","FAQ"].map(l=><button key={l} onClick={()=>{setMobNav(false);document.getElementById(l.toLowerCase())?.scrollIntoView({behavior:"smooth"})}} style={{background:"none",padding:"12px 0",fontSize:15,color:t.text,fontWeight:500,textAlign:"left",borderBottom:`1px solid ${t.surfaceBorder}`}}>{l}</button>)}
        </div>
        <div style={{padding:"12px 24px 20px",display:"flex",gap:10}}>
          <button onClick={()=>{setMobNav(false);setModal("login")}} style={{flex:1,padding:"12px 0",borderRadius:12,background:"transparent",border:`1px solid ${t.surfaceBorder}`,color:t.text,fontSize:14,fontWeight:600}}>Log In</button>
          <button onClick={()=>{setMobNav(false);setModal("signup")}} style={{flex:1,padding:"12px 0",borderRadius:12,background:t.btnPrimary,color:"#fff",fontSize:14,fontWeight:600,border:"none"}}>Get Started</button>
        </div>
      </div>
