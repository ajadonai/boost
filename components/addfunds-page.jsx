          ...
          <div className="w-[280px] shrink-0 flex">
            <div className="flex-1 flex flex-col rounded-[14px] p-[22px]" style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(255,255,255,.85)", border: `0.5px solid ${t.cardBorder}` }}>
              ...
              {gatewaysLoading ? (
                ...
                <select value={method} onChange={e => setMethod(e.target.value)} className="w-full py-2.5 px-3.5 rounded-[10px] text-sm font-medium outline-none appearance-none cursor-pointer pr-8" style={{ backgroundColor: dark ? "rgba(255,255,255,.08)" : "#fff", border: `1px solid ${dark ? "rgba(255,255,255,.14)" : "rgba(0,0,0,.18)"}`, color: t.text, fontFamily: "'Plus Jakarta Sans',sans-serif", backgroundImage: `url("data:image/svg+xml,%3Csvg width='12' height='12' viewBox='0 0 24 24' fill='none' stroke='${dark ? "%23666" : "%23999"}' stroke-width='2' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M6 9l6 6 6-6'/%3E%3C/svg%3E")`, backgroundRepeat: "no-repeat", backgroundPosition: "right 12px center" }}>
