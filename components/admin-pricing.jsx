...
export default function AdminPricingPage({ dark, t }) {
  ...
  const inpS = { padding: "9px 12px", borderRadius: 8, background: dark ? "rgba(255,255,255,.04)" : "#fff", borderWidth: "0.5px", borderStyle: "solid", borderColor: dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.1)", color: t.text, fontSize: 15, fontFamily: "'JetBrains Mono',monospace", outline: "none", textAlign: "right", boxSizing: "border-box" };
    ...
    <div style={{ padding: "0 0 40px" }}>
        ...
        {brackets.map((b, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < brackets.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)"}` : "none" }}>
              ...
              <input value={b.multiplier} type="number" step="0.05" min="1" max="10" onChange={e => { const v = parseFloat(e.target.value); if (!isNaN(v) && v >= 1) { const n = [...brackets]; n[i] = { ...b, multiplier: v }; setBrackets(n); } }} style={{ ...inpS, width: 64 }} />
      ...
      <div className="adm-card" style={{ ...cardS, padding: 20, marginBottom: 16 }}>
        ...
        <Row label="Minimum margin" hint={`You keep at least ${floorPct}% of every sale`}>
          <input value={floorPct} onChange={e => setFloorPct(Math.max(0, Math.min(90, Number(e.target.value) || 0)))} type="number" style={{ ...inpS, width: 64 }} />
      ...
      <div className="adm-grid-2" style={{ marginBottom: 16 }}>
        <div className="adm-card" style={{ ...cardS, padding: 20 }}>
          ...
          <Row label="Bonus markup" hint="Added on top of the bracket sell price">
            <input value={ngBonus} onChange={e => setNgBonus(Math.max(0, Math.min(200, Number(e.target.value) || 0)))} type="number" style={{ ...inpS, width: 64 }} />
            <span style={{ fontSize: 14, color: t.textMuted }}>%</span>
          </Row>
        </div>
